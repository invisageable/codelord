//! Zig symbol extraction for code folding.
...
/// Query for Zig symbol extraction.
const ZIG_SYMBOLS_QUERY: &str = r#"
  ; Functions
  ...
  (for_expression) @for_expr
  (for_statement) @for_stmt
  (while_expression) @while_expr
...
static ZIG_SYMBOLS_Q: LazyLock<Query> = LazyLock::new(|| {
  Query::new(&tree_sitter_zig::LANGUAGE.into(), ZIG_SYMBOLS_QUERY).unwrap()
});

/// Extract symbols from Zig source code for folding.
...
pub fn extract_symbols_from_tree(
  ...
  while let Some(matched) = matches.next() {
    ...
    map.add(SymbolAnchor {
      ...
      end_line,
      is_foldable: true,
      fold_end_inclusive: false, // Zig: show closing brace
      display_text,
      highlight_ranges,
    });
  }
