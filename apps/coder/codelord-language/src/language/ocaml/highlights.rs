//! OCaml syntax highlighting using tree-sitter-ocaml.
...
static OCAML_PARSER: LazyLock<Mutex<Parser>> = LazyLock::new(|| {
...
static OCAML_QUERY: LazyLock<Query> = LazyLock::new(|| {
  Query::new(
...
/// Parse OCaml source code and extract tokens for syntax highlighting.
pub fn parse(source: &str) -> Vec<Token> {
  if source.len() > 1_000_000 {
    return Vec::with_capacity(0);
  }

  let mut parser = OCAML_PARSER.lock().unwrap();
...
fn map_capture_to_token(capture_name: &str) -> Option<TokenKind> {
  Some(match capture_name {
    ...
    "type" | "type.builtin" | "module" | "constructor" => TokenKind::IdentifierType,
    "constant" | "constant.builtin" => TokenKind::IdentifierConstant,
    "boolean" => TokenKind::LiteralBool,
    "string" | "string.special" | "string.escape" | "character" => {
    ...
    "attribute" | "property" => TokenKind::Attribute,
